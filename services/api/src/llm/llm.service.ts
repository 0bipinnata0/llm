...
import { checkConstraintValidityTool, lookupEntityDefinitionTool } from "./tools/basic.tools";
...
export class LlmService {
  ...
  async toolLoopDemo(input: string) {
    ...
    const messages: BaseMessage[] = [
      ...
      new HumanMessage(`先抽取 action、constraints、entities，再按需要调用工具：${input}`),
    ];
    ...
    for (const toolCall of firstResponse.tool_calls ?? []) {
      ...
      const toolResult = await (targetTool.invoke as (input: unknown) => Promise<unknown>)(
        toolCall.args,
      );
      messages.push(
        new ToolMessage({
